...
public class RunQaTier1GroundingCheckToolHandler : IMcpToolHandler
{
    ...
    public string Description => "Runs the Tier 1 (deterministic, no-LLM, $0 cost) QA evidence-grounding check across all Complete evaluation results: flags per-criterion evidence quotes that cannot be located in the PD's own duty text. Writes an Excel report to reports/schedule-pc/qa-reports. Does not call any LLM (see Tier 2 in the QA plan for the LLM-as-judge follow-up).";
